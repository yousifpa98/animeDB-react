...
const TopAnimes = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState("tv"); // Default category
      ...
      try {
        for (let page = 1; page <= 4; page++) {
          const response = await fetch(
            `https://api.jikan.moe/v4/top/anime?page=${page}&type=${selectedCategory}&limit=25`
          );
  ...
  return (
    <div className="topListContainer">
      <h2 className="topListTitle">Top Animes</h2>
      <div className="topListFilters">
        {["tv", "movie", "ova", "special"].map((category) => (
          <button
            key={category}
      ...
      {loading ? (
        ...
        <ul className="topListUl">
          {currentPageData.map((item) => {
            const originalIndex = data.findIndex((d) => d.mal_id === item.mal_id);
            return (
